...
function getproducts(){
    ...
    if(!isset($_GET['category'])){
        if(!isset($_GET['brand'])){
    $select_query="select * from `tbl_products` order by rand() limit 0,9";
...
function get_all_products(){
    ...
    if(!isset($_GET['category'])){
        if(!isset($_GET['brand'])){
    $select_query="select * from `tbl_products` order by rand()";
    $result_query=mysqli_query($con,$select_query);
    while($row=mysqli_fetch_assoc($result_query)){
        $product_id=$row['product_id'];
        $product_title=$row['product_title'];
...
function get_unique_brands(){
    ...
    if(isset($_GET['brand'])){   
        $brand_id=$_GET['brand'];  
    $select_query="select * from `tbl_products` where brand_id=$brand_id";
    $result_query=mysqli_query($con,$select_query);
    $num_of_rows=mysqli_num_rows($result_query);
    if($num_of_rows==0){
...
function getbrands(){
    global $con;
    $select_brands="SELECT * FROM `tbl_brands`";
...
function getcategories(){
    global $con;
    $select_category="SELECT * FROM `tbl_categories`";
    ...
    while($row_data=mysqli_fetch_assoc($result_category)){
        ...
        echo " <li class='nav-item '>
        <a href='index.php?category=$cat_id' class='nav-link text-light'>$category_title</a>
        </li> ";
    }
}
...
function search_products(){
    ...
    $num_of_rows=mysqli_num_rows($result_query);
    if($num_of_rows==0){
        echo "<h2 class='text-center text-danger'>No matching products found.</h2>";
    }
    while($row=mysqli_fetch_assoc($result_query)){
        $product_id=$row['product_id'];
        $product_title=$row['product_title'];
